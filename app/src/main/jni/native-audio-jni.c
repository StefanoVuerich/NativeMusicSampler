...
void Java_com_example_nativeaudio_NativeAudio_initLame(JNIEnv* env, jclass clazz, jstring file)
{
    ...
    FILE *pcm = fopen(utf8, "rb");
    if(pcm == NULL) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "File nullo");
    }

    // release the Java string and UTF-8
    (*env)->ReleaseStringUTFChars(env, file, utf8);
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Lame is ok");
